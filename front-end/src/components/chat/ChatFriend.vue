...
export default {
  ...
  methods: {
    ...
    async initializeChat() {
      ...
      // 개인 채팅 고유 roomId 생성
      const generateRoomId = (userId, friendId) => {
        const sortedIds = [userId, friendId].sort((a, b) => a - b); // 정렬
        return sortedIds.map(id => String(id).padStart(3, '0')).join(''); // 패딩 후 결합
      };
      this.roomId = generateRoomId(this.userId, this.friendId);

      // 친구 이름 가져오기
      const response = await axios.get(`http://localhost:9090/api/profiles/${this.friendId}`);
      this.friendName = response.data.name;

      // 사용자 이름 캐시
      const userProfile = await axios.get(`http://localhost:9090/api/profiles/${this.userId}`);
      this.cachedProfileName = userProfile.data.name;
